...
def visualize_flow_submission(seq_name: str, flow: numpy.ndarray, file_index: int):
...
def validate_dsec(model,
    ...
    config = json.load(open("raft_stuff/config/dsec_standard.json"))
    # Create Save Folder
    ...
    loader = DatasetProvider(
        ...
        dataset_path=Path("/home/anca/Documents/E-RAFT-main/DSEC"),
    ...
    val_dataset = DataLoader(test_set,
    ...
    # TODO: add flag for this or something
    downsample = False
    for batch_idx, batch in enumerate(val_dataset):
        # TODO: try with warmstarting too
        if not downsample:
            im1 = batch['event_volume_old'][0]
            im2 = batch['event_volume_new'][0]
        else:
        ...
        image1 = im1[None].cuda()
        image2 = im2[None].cuda()
        ...
        flow_pr = results_dict['flow_preds'][-1][0]
        
        print("Sample {}/{}".format(batch_idx + 1, len(val_dataset)))

        for batch_idx in range(len(batch['file_index'])):
            if batch['save_submission'][batch_idx]:
                print ("Saving this sample ...")
                sequence_name = visu_add_args['name_mapping'][int(batch['name_map'][batch_idx].item())]
                # Save for Benchmark Submission
                visualize_flow_submission(
...
def validate_chairs(model,
    ...
    for val_id in range(len(val_dataset)):
        ...
        print(f"entry {val_id}: epe={epe.view(-1).numpy()}")
